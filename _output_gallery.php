...
function fpf_fetch_album_content($aid, $params)
{
    ...
    {
        $retVal['content'] = "<i>Warning: An unknown error occurred while trying to fetch the photos (the album reported $album->count entries, but only " . count($photos->data) . " were returned).</i><br />";
    ...
    //Output a warning against tags containing the old isPage/isGroup params.
    if( $params['isGroup'] )
        $retVal['content'] = "<i>Warning: Groups are not supported by this version of Facebook Photo Fetcher.</i><br />";
    if( $params['isPage'] )
        $retVal['content'] = "<i>Warning: As of Facebook Photo Fetcher v2.0, the 'isPage' parameter has been deprecated.  Please see the <a href='$fpf_homepage' target='_docs'>plugin documentation</a> for updated information on how to fetch fanpage albums.</i><br />";
