...
/**
  ...
  * $retVal['hideCred'] //Omit the "Generated by Facebook Photo Fetcher" footer (please don't, though :))
  * $retVal['rand']     //Display a n random photos from the album (or between "start" and "max", if specified)
  */
...
function fpf_fetch_album_content($aid, $params)
{
    ...
    //Get the photos, and slice out the requested items
    $photos = $facebook->api_client->photos_get(null, $aid, null);
    $photos = array_slice($photos, $params['start'], $params['max']); 
    
    //If "rand" is specified, randomized the order and slice again
    if( $params['rand'] )
    {
        shuffle($photos);
    ...
    if(!$params['hideHead'])
    {
        ...
        if( $actualCount < $album['size'])$headerTitle .= ' (Showing ' . $actualCount . ' of ' . $album['size'] . " items)\n";
    ...
    $retVal['content'] .= "<!-- Album ". $aid ." Last fetched on " . date('m/d/Y H:i:s') . "-->\n";
